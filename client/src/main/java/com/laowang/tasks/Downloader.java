...
public class Downloader {
    ...
    private static String url="http://localhost:2568/multiDownload/download";
    ...
    public static String download(String serverFilePath, Integer length) {
        ...
        String storeFile = storeDir + File.separator +fileName + ".temp";
        File file = new File(storeFile);
        //2.保存文件不存在的话，创建它
        if (!file.exists() || !file.isFile()) {
            FileUtil.createFile(storeFile);
        }
